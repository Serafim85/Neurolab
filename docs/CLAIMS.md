...
| # | Заявление | Источник | Оговорка | Уровень |
|---|---|---|---|---|
| **C-50** | «Юнит-набор песочницы: **51 passed** (`pytest -m "not integration"`)» | `STATUS.md` session log 2026-08-08 | Снимок конкретного прогона на Mac, не CI: **CI в репозитории нет**. Это тесты нашего кода, а не валидация научных результатов. `docs/CLOSED-SANDBOX-VERIFY.md` и MVP §9 всё ещё говорят «11» — при цитировании сверяться с прогоном, а не с этими листами | `public` |
...
| **C-53** | — | `models/outpost-tiny/CARD.md` (генерируемый блок) | Upstream LICENSE базовой модели **в репозитории не зафиксирована**. До решения человека паспорт неполон по `AGENTS.md` §5.7, и любое заявление о правах на распространение весов запрещено | `internal` |
