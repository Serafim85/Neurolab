...
> Живой статус и **текущий фокус**: `STATUS.md` · Архитектура: `docs/ARCHITECTURE.md` · Решения: `docs/DECISIONS.md`
...
| **`STATUS.md`** | **единственный источник фокуса** + Session log (с 2026-08-01) |
| `docs/SESSIONS-2026-07.md` | архив Session log за июль 2026 (только чтение) |
...
3. Фокус — **только `STATUS.md`** §Summary + §Next.
   `docs/ARCHITECTURE.md` — архитектурный контекст, не фокус.
...
5. Взять **одну** работу из `STATUS.md` §Next (сверху — приоритетнее).
...
1. Обновить `STATUS.md`:
   - §Summary / §Ladder — если изменилось состояние; §Next — снять сделанное, дописать новое
   - **Session log** (2–8 строк): что сделано, как проверить, следующие шаги
   - журнал растёт вверх; записи старше текущего месяца → `docs/SESSIONS-<YYYY-MM>.md`
...
**Единственный источник правды — [`STATUS.md`](STATUS.md) §Summary + §Next.**
Здесь фокус не дублируется: копия расходится с оригиналом за одну сессию —
именно так §6 успела застрять на мёртвой шкале `/16` (актуальная — `/20`).

Что не меняется независимо от фокуса: следующий размер / слой / эксперт
открывается только после **измеренного** результата на текущем
(`docs/SCALE-PLAN.md`), и обучения на ПДн заказчика не бывает никогда (§8–§9).
